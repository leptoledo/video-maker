...
def criar_draft_imagens(imagens, audio_path, audio_dur_seg, pasta_destino,
                        nome_projeto="LctnetVideo_Cenas",
    ...
    for gi, (ts, paths) in enumerate(grupos):
        ...
        for k, img_path in enumerate(paths):
            ...
            w, h = _dims_imagem(img_path)

            # Material photo (clona do real, troca campos variáveis)
            mphoto = copy.deepcopy(ref["material_photo"])
            mphoto["id"] = _novo_id()
            mphoto["path"] = str(Path(img_path).resolve()).replace("\\", "/")
            mphoto["material_name"] = Path(img_path).name
    ...
    # No Mac, o CapCut mais recente usa draft_info.json em vez de draft_content.json
    import sys
    nome_json_draft = "draft_info.json" if sys.platform == "darwin" else "draft_content.json"
    with open(pasta_draft / nome_json_draft, "w", encoding="utf-8") as f:
